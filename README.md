# Pong

Classic Pong Game: A recreation of the iconic Pong game where players control paddles to bounce a ball back and forth. Enjoy classic gameplay in a nostalgic 1 or 2-player mode.

## How to Play

- Use the arrow keys to select mode
- Use W S for Player 1
- Use Up Down arrows for Player 2

## Screenshots
<div style="display: flex; justify-content: center;">
  <img src="https://github.com/MonikaBukov/pong/assets/135535997/75e8898f-23f7-4f55-8342-e5982c19651e" width="400" alt="Image 1">
  <img src="https://github.com/MonikaBukov/pong/assets/135535997/a4cda051-1d6e-4619-a71c-ab4481c5847e)" width="400" alt="Image 2">
</div>

## Demo
[![Watch the video](https://i.imgur.com/vKb2F1B.png)]([https://youtu.be/vt5fpE0bzSY](https://www.youtube.com/watch?v=nce04Y1PjnQ))

## Technologies Used

- C++
- SFML

## Credits

- Game assets and resources provided by UWE.

## License

This project was created solely for a school assignment and is not intended for public release or distribution. All rights to the game and its code belong to the project creator.


